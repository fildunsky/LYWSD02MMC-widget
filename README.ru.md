...
Скачайте `lywsd02-widget-macos-arm64.zip` со [страницы релизов](../../releases) (Apple Silicon; на Intel-маках можно запустить из исходников: `pip install bleak rumps pillow && python3 mac/lywsd02_widget_mac.py`), распакуйте и запустите. Приложение не подписано, поэтому первый запуск — через правый клик → «Открыть», затем macOS спросит разрешение на Bluetooth. Живёт в строке меню: там может быть иконка или живой текст — работают все шесть режимов, включая `23.8°, 48% 🙂`. Настройки — подменю выпадашки (в стиле macOS); автозапуск — LaunchAgent; конфиг — `~/Library/Application Support/lywsd02-widget/config.json`. Нюанс: macOS скрывает Bluetooth MAC-адреса, устройства опознаются по системным UUID — выбирайте часы встроенным поиском.

### Windows

Скачайте `lywsd02-widget.exe` со [страницы релизов](../../releases) и просто запустите — установка не нужна (Windows 10+ с BLE-адаптером). Иконки в трее Windows квадратные, поэтому там показывается иконка, мордашка, температура или влажность, а полные показания — во всплывающей подсказке и в меню. Настройки, поиск часов, синхронизация времени и автозапуск (ключ Run в реестре) работают как в Linux; конфиг — `%APPDATA%\lywsd02-widget\config.json`. SmartScreen может предупредить о неподписанном файле — «Подробнее → Выполнить в любом случае». Сборка идёт в CI (workflow `windows-build`) из `win/lywsd02_widget_win.py`.
